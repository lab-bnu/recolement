...
def find_one_file(path):
    ...
    df1_final = df1.dropna(subset=['Etat copie', 'Précision exemplaire'], how='all')
    df1_final = pd.concat([df1_final, df1[df1['Description']=="Exemplaires absents dans le fichier"]], ignore_index=True, axis=0)
    df1_final = pd.concat([df1_final, df1[(df1['Description']=="Exemplaires en prêt")]], ignore_index=True, axis=0)
    df1_final = pd.concat([df1_final, df1[(df1['Description']=="Exemplaire avec demandes ou réservations")]], ignore_index=True, axis=0)
    df1_final = pd.concat([df1_final, df1[(df1['Type circulation']!="Prêt externe")]], ignore_index=True, axis=0)

    df1_final = pd.concat([df1_final, df1[(df1['Code-barres'].str.len() != 14)]], ignore_index=True, axis=0) # n'a pas 14 caractères
    df2_final = df2.dropna(subset=['Etat copie', 'Précision exemplaire'], how='all')
    df2_final = pd.concat([df2_final, df2[(df2['Description']=="Exemplaire avec cote hors de l'intervalle")]], ignore_index=True, axis=0)
